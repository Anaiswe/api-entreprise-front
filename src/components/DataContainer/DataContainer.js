...
const DataContainer = ({ theme }) => {
  ...
  const [showAdditionalInfo, setShowAdditionalInfo] = useState(Array(0).fill(false));
  ...
  const toggleAdditionalInfo = (index) => {
    const newShowAdditionalInfo = [...showAdditionalInfo];
    newShowAdditionalInfo[index] = !newShowAdditionalInfo[index];
    setShowAdditionalInfo(newShowAdditionalInfo);
  };
          ...
          {data[0]?.map((item, index) => (
            <div className={`card${className}`} key={item.id}>
              <div className={`card-content${className}`}>
                ...
                <div className="btn-infos-display" onClick={() => toggleAdditionalInfo(index)}>
                  {showAdditionalInfo[index] ? "Masquer" : "Afficher plus"}
                </div>
